...
function WdtSub1Menu
{
	...
	for i in $(ls -v $SYSFS); do
		local dev=`wdctl /dev/${i} 2> /dev/null | grep eio`
		if [ "$dev" != "" ]; then
			wdt=${i}
		fi
	done
	...
	if [ "$wdt" == "" ]; then
		return `read -p"Can't find eio watchdog"`
	fi
...
function HwmSub1Menu
{
	...
	do
		if [ -e $SYSFS/$dev/name ] &&
		   [ `cat $SYSFS/$dev/name` == "eio_hwmon" ]; then
			hwm_dev=$SYSFS/$dev
			break
		fi
	done
	...
	if [ $hwm_dev == ""]; then
		return `read -p"Can't find eio hwmon device..."`
	fi
...
function SmartFanSub1Menu
{
	...
	for dev in $(ls -v $SYSFS); do
 		if [ -e $SYSFS/$dev/type ] &&
		   [ `cat $SYSFS/$dev/type` == "CPU0" ] &&
		   [ -e $SYSFS/$dev/fan_mode ]; then
			names[${#names[@]}]=$dev
		fi
	done
	...
	while : ; do
		...
		echo -e "3) Manual PWM		:" `cat $tz_dev/cdev0/cur_state`
		...
		"3")
			...
			echo "$val" > $tz_dev/cdev0/cur_state
			;;
		"4")
...
function ThermalSub1Menu
{
	...
	do
		...
		echo -e " ) Sensor		:" `cat ${fs}/type`
		echo -e " ) Temperature		:" $((temp / factor)).$((temp % factor))
		echo -e " ) Type			:" `cat $fs/trip_point_${act}_type`
		echo -e "2) Event Type		:" ${ACTS[$act]}
		echo -e "3) Trigger Temperature	:" $((trigger / factor)).$((trigger % factor))
		...
		"3")
			...
			echo $((trigger * factor)) > $trigger_fs
			;;
		"4")
...
function GpioSub1Menu
{
	...
	for base in $(ls -v $SYSFS); do
		if [ -e $SYSFS/${base}/label ] &&
		   [ `cat $SYSFS/${base}/label` == "gpio_eio" ] &&
		   [ -e $SYSFS/${base}/base ]; then
			# get rid of prefix gpiochip
			ngpio=` cat $SYSFS/${base}/ngpio `
			base=` cat $SYSFS/${base}/base `
			break
		fi
	done
...
function BlSub1Menu
{
	...
	local PARAM=/sys/module/eio_bl/parameters
	local i
	local val
	local bl_name

	# find the Backlight
	for i in $(ls -v $SYSFS | grep eio_bl); do
		bl_name=$i
		break ;
	done

	if [ "$bl_name" == "" ]; then
		return `read -p"Can't find eio bl"`
	fi
	...
	while :	; do
		...
		"2")
			...
			if [ $val -ge 10 ]; then
				rmmod eio_bl
				modprobe eio_bl bri_freq=${val}
				sleep 0.2
			fi
			;;
		"3")
			rmmod eio_bl
			modprobe eio_bl bri_invert=$((1 - (bri_invert&1)))
			sleep 0.2
			;;
		"4")
		...
		"5")
			rmmod eio_bl
			modprobe eio_bl bl_power_invert=$((1 - bl_power_invert))
			sleep 0.2
			;;
		*)
			;;
...
function SMBusSub1Menu
{
	...
	for i in $(ls -v $SYSFS); do
		if [ -d $SYSFS/$i ] &&
		   [ "$(cat $SYSFS/$i/name | grep "eio-smbus")" != "" ]; then
	...
	while : ; do
		...
		echo -e " ) Internal name: `cat $SYSFS/i2c-${hosts[$cur]}/name | sed 's/eio //g'`"
		echo -e "2) Probe"
		echo -e "3) Read"
		echo -e "4) Write\n"
		read -n1 -p "Enter your choice: " sel
...
{
	local SYSFS=/sys/bus/i2c/devices
	local PARAM=/sys/module/i2c_eio/parameters
	local i=0
	local hosts=()
	local val
	...
	for I2CN in $(ls -v $SYSFS); do
		if [ -d $SYSFS/$I2CN ] &&
		   [ "$(cat $SYSFS/$I2CN/name | grep "eio-i2c")" != "" ]; then
	...
	while : ; do
		local name=$(cat $SYSFS/i2c-${hosts[$i]}/name | sed 's/eio-//g')
		...
		"6")
			...
			if [ -n "$val" ]; then
				rmmod i2c_eio
				modprobe i2c_eio ${name}_freq=${val}
			fi
			;;
		esac
	done
...
{
	local DIR=/sys/bus/isa/devices/eio_core.0
...
function MainMenu
{
	local DRIVERS=( eio_core eio_wdt eio_hwmon eio_fan eio_thermal eio_bl gpio_eio i2c_eio )
	local loads=()
	...
	do
		...
		echo -e "Main (EIO driver demo script)"
		echo -e
